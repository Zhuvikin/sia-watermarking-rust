...
use crate::image_info::WatermarkedImage;
use crate::logging::log;
use crate::web_image::WatermarkedImageWeb;
use watermarking;

#[wasm_bindgen]
pub fn watermark_image(uint8array: Uint8Array) -> WatermarkedImageWeb {
    console_log!("open_image_from_array_buffer {:?}", uint8array);

    let raw_data: Vec<u8> = uint8array.to_vec();

    let result = watermark(raw_data);

    let original_js_value = JsValue::from(
        result
            .clone()
            .original
            .into_iter()
    ...
    let watermarked_js_value = JsValue::from(
        result
            .clone()
            .watermarked
            .into_iter()
    ...
    return WatermarkedImageWeb {
        width: result.width,
        height: result.height,
        format: result.format,
        original: Uint8Array::new(&original_js_value),
        watermarked: Uint8Array::new(&watermarked_js_value),
    };
}

pub fn watermark(raw_data: Vec<u8>) -> WatermarkedImage {
    let input_encoded_data = Cursor::new(raw_data);
    ...
    let (width, height) = image.clone().dimensions().clone();
    let input_pixels = image.clone().into_rgba8().into_raw();
    ...
    return WatermarkedImage {
        ...
        height,
        format: image_format,
        original: input_pixels,
        watermarked: output_pixels,
    };
}
...
fn watermark_test_bmp() {
    ...
    let actual = watermark(image_array);
...
fn watermark_test_tiff() {
    ...
    let actual_lwz = watermark(image_array_lzw);
    assert_eq!(expected.width, actual_lwz.width);
    assert_eq!(expected.height, actual_lwz.height);
    assert_eq!(expected.format, actual_lwz.format);
}
